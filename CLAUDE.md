...
**Current phase**: Phase 1 complete (632 tests passing: 68 policy-engine + 564 proxy). Shipped: proxy core, policy packs (D-036), hook endpoint (D-040 Phase A), cli-protection + observe-mode, PocketOS incident demo (rind-demo repo), credential redaction (cred-010), scanner data-exposure patterns. **Now building**: MCP server registration API (D-049) — `POST /servers`, agent key issuance (D-050), phantom token validation per request. This unblocks the demo MCP connectivity gap and the D-047 routing wedge. D-051: DPoP moved to Phase 2 enterprise (neither Aembit nor API Stronghold ships it; Claude Code doesn't support it). D-052: active probe / `npx rind-scan` planned after server registration.
